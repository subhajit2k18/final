...
double phi2(int i,double zeta,double eta,double zi);
double d_phi2(int i,double zeta,double eta,double zi,char type);

 void element_stiffness2(int elementno,float *d_material_data,float *elemental_data,int node_index,int mxelement,float * r_nodal_data)
{
	float jacobian[9];
	float matrix_b[144];
...
///////////////////////////keeping values of derivative of shape function in an array
	for(int i=0;i<24;i++)
{	if(i<8)
	d_shapefn[i]=d_phi2(i+1,gausspoints[m],gausspoints[n],gausspoints[p],'z');
	else if(i>=8 && i<16)
	d_shapefn[i]=d_phi2(i-7,gausspoints[m],gausspoints[n],gausspoints[p],'e');
	else 
	d_shapefn[i]=d_phi2(i-15,gausspoints[m],gausspoints[n],gausspoints[p],'j');
}
...
//__syncthreads();	
	for(int j=0;j<8;j++)
		...
		float x,y,z;
		x=r_nodal_data[node_index*24+j*mxelement*3+elementno];
		y=r_nodal_data[node_index*24+j*mxelement*3+elementno+mxelement];
		z=r_nodal_data[node_index*24+j*mxelement*3+elementno+2*mxelement];
